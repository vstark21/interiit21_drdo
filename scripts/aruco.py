...
class Aruco_Land():
    ...
    def __init__(self):
        ...
        self.Factor = 0.0046 # Factor to scale between image co-ordinate system and mavros co-ordinate system
    ...
    def Perpendicular_Distance(self, Line, Point):
        return abs((Line[0] * Point[0]) + (Line[1] * Point[1]) + Line[2]) / self.Absolute_Value(Line) 
    ...
    def Initialize_Limits(self, yaw, Position):
        self.Limits_Initialized = True

        x1, y1 = self.MAVROS_Position(yaw, Position, [320, 0])
        x2, y2 = self.MAVROS_Position(yaw, Position, [320, 480])
        ...
        x1, y1 = self.MAVROS_Position(yaw, Position, [0, 240])
        x2, y2 = self.MAVROS_Position(yaw, Position, [640, 240])
    ...
    def Aruco(self, yaw, image):
        ...
        for i, corner in enumerate(corners):
            ...
            Corners.append(corner[0].tolist())

            if ids[i][0] == 0:
                self.Landing = True
                self.Aruco_Position = self.MAVROS_Position(yaw, Position, [x, y])
                return None, None
    ...
    # To convert the image co-ordinates into the mavros co-ordinate system
    def MAVROS_Position(self, yaw, Position, centre):
        del_x = centre[0] - 320
        del_y = 240 - centre[1]
    
        Length = self.Euclidean_Distance(centre, [320, 240])

        image_angle_1 = math.acos(del_y / Length)
        image_angle_2 = math.asin(del_x / Length)
        
        if del_x <= 0 and del_y <= 0:
            image_angle = image_angle_1 + math.pi
        elif del_x <= 0 and del_y >= 0:
            image_angle = image_angle_2
        else:
            image_angle = image_angle_1

        actual_angle = image_angle + yaw + math.pi
        
        factor_x = abs(math.cos(actual_angle))
        factor_y = abs(math.sin(actual_angle))
        ...
        real_length = Length * self.Factor * Position[2]
    
        real_x = (real_length * factor_x) + Position[0]
        real_y = (real_length * factor_y) + Position[1]
    ...
    def No_Optimal_Point(self, yaw, Position):
        Left_Distance = self.Perpendicular_Distance(self.Left, Position)
        Right_Distance = self.Perpendicular_Distance(self.Right, Position)
        ...
        if (self.Last_Move == 'R' and Left_Distance < self.Threshold_Distance) or (self.Last_Move == 'L' and Right_Distance < self.Threshold_Distance) or self.Current_Front_Distance > self.Threshold_Distance:
            if self.Switch:
                ...
                x, y = self.MAVROS_Position(yaw, Position, [320, 0])
            ...
            x, y = self.Point_of_Intersection(self.Front, Position)
            self.Current_Front_Distance = self.Perpendicular_Distance(self.Front, Position)    

        elif self.Last_Move == 'L':
            x, y = self.Point_of_Intersection(self.Right, Position)
            self.Switch = True
        elif self.Last_Move == 'R':
            x, y = self.Point_of_Intersection(self.Left, Position)
            self.Switch = True
    ...
    def Main(self, yaw, Position, image):
        if self.Landed:
            return None

        if not self.Landing:
            Centres, Corners = self.Aruco(yaw, image)
        
        if self.Landing:
            x, y = self.Aruco_Position
            if self.Euclidean_Distance([x, y], Position) > 0.5:
                return [x, y, self.Drone_Height]
            elif self.Euclidean_Distance([x, y], Position) > self.Threshold_Distance:
                return [x, y, 2.0]
            else:
                self.Landed = True
                return [x, y, 0.0]
        ...
        Unvisited = []
        for Centre, Limit in zip(Centres, Corners):
            mavros_position = self.MAVROS_Position(yaw, Position, Centre)
            Flag = False
            for P in self.Visited_Aruco:
                if self.Euclidean_Distance(P, mavros_position) < self.Aruco_Length:
                    Flag = True
                    break
            if not Flag:
                List = [self.Euclidean_Distance(Position, mavros_position), mavros_position]
                for Position in Limit:
                    List.append(self.MAVROS_Position(yaw, Position, Position))
        ...
        if len(Unvisited):
            ...
            if not self.Limits_Initialized:
                self.Initialize_Limits(yaw, Position)

            return [x, y, self.Drone_Height]

        Centres = self.White_Points(image)
        ...
        for Centre in Centres:
            ...
            for P in self.Visited_Aruco:
                if self.Euclidean_Distance(P, mavros_position) < self.Aruco_Length:
                    ...
                    break
            if not Flag:
                Unvisited.append([self.Euclidean_Distance(Position, mavros_position), mavros_position])
        ...
        if self.Limits_Initialized:
            return self.No_Optimal_Point(yaw, Position)
